...
def threshold(filename):
    ...
    threshold = filename['Mean'][(len(filename.index) - 5):
                                 len(filename.index)].mean(axis=0)
    return threshold
...
if __name__ == "__main__":
    ...
    while (i <= len(os.listdir())):
        ...
        for n in np.arange(1, 8):
            ...
            ArcCells = ArcCount['Mean'][
                (ArcCount['Mean'] > (
                    threshold(ArcCount) *
                    timesbaseline))]
            Slice['Cell Density'][0] = (
                len(NeuNCells.index) + 1) / (NeuNCount['Area'][1])
            ...
            Slice['Density'][0] = Slice['Number'][
                0] / (Slice['Area'][0] / 44696.932)
            Slice['Proportion'][0] = Slice['Density'][
                0] * Slice['Cell Density'][0]
        ...
        PostLeft = PostLeft.append(Amygdala[(int(
            len(Amygdala.index) / 2)):len(Amygdala.index)].mean(axis=0), ignore_index=True)
        os.chdir(os.pardir)
        i = i + 1
    ...
    while (i <= len(os.listdir())):
        ...
        AntRight = AntRight.append(
            Amygdala[0:(int(len(Amygdala.index) / 2))].mean(axis=0), ignore_index=True)
        PostRight = PostRight.append(Amygdala[(int(
            len(Amygdala.index) / 2)):len(Amygdala.index)].mean(axis=0), ignore_index=True)
        os.chdir(os.pardir)
        i = i + 1
    ...
    AverageDensity = pd.DataFrame(
        [
            CombinedMice['Density'][
                0:4].tolist(), CombinedMice['Density'][
                19:25].tolist(), CombinedMice['Density'][
                    10:19].tolist(), CombinedMice['Density'][
                        4:10].tolist()], index=[
                            'HC', '.3 mA', '.5 mA', '.75 mA']).transpose()
    AverageArea = pd.DataFrame(
        [
            CombinedMice['Area'][
                0:4].tolist(), CombinedMice['Area'][
                19:25].tolist(), CombinedMice['Area'][
                    10:19].tolist(), CombinedMice['Area'][
                        4:10].tolist()], index=[
                            'HC', '.3 mA', '.5 mA', '.75 mA']).transpose()
    AverageProportion = pd.DataFrame(
        [
            CombinedMice['Proportion'][
                0:4].tolist(), CombinedMice['Proportion'][
                19:25].tolist(), CombinedMice['Proportion'][
                    10:19].tolist(), CombinedMice['Proportion'][
                        4:10].tolist()], index=[
                            'HC', '.3 mA', '.5 mA', '.75 mA']).transpose()
    LeftDensity = pd.DataFrame(
        [
            MiceLeft['Density'][
                0:4].tolist(), MiceLeft['Density'][
                19:25].tolist(), MiceLeft['Density'][
                    10:19].tolist(), MiceLeft['Density'][
                        4:10].tolist()], index=[
                            'HC', '.3 mA', '.5 mA', '.75 mA']).transpose()
    RightDensity = pd.DataFrame(
        [
            MiceRight['Density'][
                0:4].tolist(), MiceRight['Density'][
                19:25].tolist(), MiceRight['Density'][
                    10:19].tolist(), MiceRight['Density'][
                        4:10].tolist()], index=[
                            'HC', '.3 mA', '.5 mA', '.75 mA']).transpose()
